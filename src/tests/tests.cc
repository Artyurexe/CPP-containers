...
TEST(MultisetTest, Count) {
  ...
  EXPECT_EQ(ms.count(2), size_t(2));
  EXPECT_EQ(ms.count(4), size_t(0));
  EXPECT_EQ(ms.count(1), size_t(1));
  EXPECT_EQ(ms.count(3), size_t(1));
}
...
TEST(ArrayTest, MoveConstructor) {
  ...
  EXPECT_EQ(size_t(3), arr1.size());
  EXPECT_EQ(size_t(3), arr2.size());
  EXPECT_EQ(1, arr2[0]);
  EXPECT_EQ(3, arr2[2]);
}

TEST(ArrayTest, Destructor) { s21::s21_array<int, 3> arr = {1, 2, 3}; }
...
TEST(ArrayTest, Iterator) {
  s21::s21_array<int, 3> arr = {1, 2, 3};
  s21::s21_array<int, 3>::iterator it = arr.begin();
  EXPECT_EQ(1, *it);
  ++it;
  EXPECT_EQ(2, *it);
  it++;
